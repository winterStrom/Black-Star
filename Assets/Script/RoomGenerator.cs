...
public class RoomGenerator : MonoBehaviour
{
    ...
    {   
        ...
        for (int i = 0; i < roomNumber; i++)
        {
            if(i == 0)
            {
                rooms.Add(Instantiate(roomPrefab[0],generatorPoint.position, Quaternion.identity).GetComponent<Room>());
                ChangePointPos();
            }
            else
            {
                rooms.Add(Instantiate(roomPrefab[Random.Range(1,roomPrefab.Length-1)], generatorPoint.position, Quaternion.identity).GetComponent<Room>());
                ChangePointPos();
            }
            //改变point位置
            //ChangePointPos();
        }
        //rooms[0].GetComponent<GroundController>().fristNumber = 0 ;
        //rooms[0].GetComponent<GroundController>().secondNumber = 0 ;
        rooms[0].GetComponent<SpriteRenderer>().color = startColor;//改变第1个房间的颜色
        //rooms[0].GetComponent<EnemyController>().enemyTotalNum = 0;
    ...
    private void BossRoom()
    {
       ...
       endRoom = Instantiate(roomPrefab[3],endRoom.transform.position,Quaternion.identity);
    }
    private void SetNextDoor()
    {
        endRoom.GetComponent<Room>().Door.SetActive(true);
    }
}
[System.Serializable]
